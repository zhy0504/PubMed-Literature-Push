#!/bin/bash
# PubMed Literature Push Launcher for Linux
...
show_help() {
    echo -e "${CYAN}===================================="
    echo -e "${CYAN}  PubMed Literature Push Launcher for Linux (Cross-Platform)"
    echo -e "${CYAN}====================================${NC}"
    echo ""
    echo -e "${YELLOW}Usage:${NC}"
    echo -e "${WHITE}  ./launcher_linux.sh [Options]${NC}"
    echo ""
    ...
    echo -e "${WHITE}  ./launcher_linux.sh                    # Quick launch${NC}"
    echo -e "${WHITE}  ./launcher_linux.sh --setup            # Complete environment setup${NC}"
    echo -e "${WHITE}  ./launcher_linux.sh --action start     # Start background service${NC}"
    echo -e "${WHITE}  ./launcher_linux.sh --action run       # Run main program${NC}"
    echo ""
}
...
start_gui() {
    ...
    if [ -z "$DISPLAY" ]; then
        log "ERROR" "No display available. GUI cannot be started."
        log "INFO" "Make sure you are running this script in a graphical environment or use X11 forwarding."
        return 1
    fi
...
[Unit]
Description=PubMed Literature Push Service
...
Restart=always
RestartSec=10
StandardOutput=file:$PROJECT_ROOT/logs/autostart.log
StandardError=file:$PROJECT_ROOT/logs/autostart_error.log
...
# Disable auto-start on boot (Linux systemd)
disable_autostart() {
...
main() {
    ...
    # Display system info
    if is_wayland; then
        log "INFO" "Running on Wayland display server"
    else
        log "INFO" "Running on X11 display server"
    fi
