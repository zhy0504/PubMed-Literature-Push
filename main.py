...
def run_job():
    ...
    try:
        ...
        for i, (keyword, emails) in enumerate(keyword_to_emails.items()):
            ...
            logging.info("开始批量翻译摘要...")
            pubmed_processor.translate_abstracts_in_batch(articles)

            logging.info(f"为 '{keyword}' 的 {len(articles)} 篇文章生成综述...")
            logging.info(f"文章列表: {[article.get('pmid', 'N/A') for article in articles]}")
            raw_review_content = pubmed_processor.generate_review(articles, keyword)
            
            logging.info("校准引用顺序并移除参考文献列表...")
            logging.debug(f"原始综述内容: {raw_review_content}")
            review_body, sorted_articles = process_review_and_sort_articles(raw_review_content, articles)
...
def process_review_and_sort_articles(review_text: str, original_articles: list) -> Tuple[str, list]:
    """
    ...
    try:
        ...
        # 2. 提取正文中引用的原始顺序
        citations_in_order = [int(c) for c in re.findall(r'\[(\d+)\]', body)]
        ...
        # 检查是否有文章未被引用
        if len(sorted_articles) < len(original_articles):
            unreferenced_articles = []
            for i, article in enumerate(original_articles):
                if i + 1 not in unique_ordered_old_refs:
                    unreferenced_articles.append(article.get('pmid', 'N/A'))
            logging.info(f"有 {len(unreferenced_articles)} 篇文章未被综述引用: {unreferenced_articles}")
        
        # 6. 更新正文中的引用标记
        def replace_citation(match):
            old_idx = int(match.group(1))
            # 使用映射表找到新的引用号，如果找不到则保持原样
            return f"[{old_to_new_map.get(old_idx, old_idx)}]"
